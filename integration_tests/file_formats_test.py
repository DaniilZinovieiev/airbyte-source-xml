...
def run_load_dataframes(config, expected_columns=10, expected_rows=42):
    df_list = SourceFileCustom.load_dataframes(config=config, logger=AirbyteLogger(), skip_data=False)
...
def run_load_nested_json_schema(config, expected_columns=10, expected_rows=42):
    data_list = SourceFileCustom.load_nested_json(config, logger=AirbyteLogger())
